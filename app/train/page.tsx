...
export default function TrainPage() {
  ...
  async function send() {
    ...
    if (tab === "teach") {
      setTeach((m) => [...m, you, { role: "ai", text: "Saved. Add a product or a policy, then switch to Test." }]);
      return;
    }
    ...
    setTest((m) => { const copy = [...m]; copy[copy.length - 1] = { role: "ai", text: data.text || "No reply" }; return copy; });
  }
  ...
  return (
    <>
      ...
      <Card>
        <div className="min-h-[280px] space-y-3">{thread.map((m, i) => <div key={i} className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${m.role === "ai" ? "bg-brand-tint text-stone-800" : "ml-auto bg-stone-900 text-white"}`}>{m.text}</div>)}</div>
