    ...
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-2">{items.map((c) => (
        <Card key={c.id}>
          ...
          <form className="mt-3 flex gap-2" onSubmit={(e) => { e.preventDefault(); const key = String(new FormData(e.currentTarget).get("key") || ""); setItems(saveConnector(c.id, key)); e.currentTarget.reset(); }}>
