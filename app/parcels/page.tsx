...
export default function ParcelsPage() {
  ...
  function onSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    logEvent("parcel", "Created " + p.tracking);
    setItems(loadParcels());
  }
  return (
    <>
      <PageHeader title="Parcels" desc="Generic courier — BYO label." />
      <Card className="mb-4">
        <form className="flex flex-wrap gap-2" onSubmit={onSubmit}>
          <select name="orderId" className="rounded-xl border px-3 py-2 text-sm"><option value="none">No order</option>{orders.map((o) => <option key={o.id} value={o.id}>{o.id}</option>)}</select>
      ...
      <Card className="p-0">{items.length === 0 ? <p className="p-8 text-center text-sm text-stone-500">No parcels.</p> : items.map((p) => (
        <div key={p.id} className="flex items-center justify-between border-b px-4 py-3 text-sm">
          <div><p className="font-medium">{p.tracking}</p><p className="text-xs text-stone-500">{p.courier} · {p.status}</p></div>
          <div className="flex gap-2">
            <button type="button" className="text-xs text-stone-500" onClick={() => navigator.clipboard.writeText(p.tracking)}>Copy</button>
            <button type="button" className="text-xs text-brand" onClick={() => setItems(advanceParcel(p.id))}>Advance</button>
          </div>
        </div>
      ))}</Card>
    </>
