...
import { CONTACT_CSV_HEADER, loadCampaigns, loadContacts, mergeContacts, parseContactsCsv, sendCampaign, type Campaign, type Contact } from "@/lib/email";
    ...
    <>
      ...
      {tab === "audience" ? (
        <Card>
          ...
          <ul className="mt-3 text-sm">{contacts.slice(0, 20).map((c) => <li key={c.email} className="border-b py-2">{c.email} · {c.first_name} {c.last_name}</li>)}</ul>
        </Card>
      ) : (
        <Card>
          <form className="grid gap-3" onSubmit={(e) => { e.preventDefault(); const data = new FormData(e.currentTarget); sendCampaign(String(data.get("name") || "Campaign"), String(data.get("subject") || ""), String(data.get("body") || "")); setCampaigns(loadCampaigns()); setLog(`Queued ${loadContacts().length}`); }}>
          ...
          <ul className="mt-4 text-sm">{campaigns.map((c) => <li key={c.id} className="border-t py-2">{c.name} · {c.recipients}</li>)}</ul>
        </Card>
      )}
    </>
  );
