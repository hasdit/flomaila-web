...
type HomeStats = { name: string; country: string; currency: string; credits: number; products: number; orders: number; sales: number; threads: number };
export default function HomePage() {
  const [s, setS] = useState<HomeStats>({ name: "Flomaila Demo", country: "US", currency: "USD", credits: 0, products: 0, orders: 0, sales: 0, threads: 0 });
  useEffect(() => {
    ...
    <>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <Card><p className="text-xs uppercase text-stone-400">Sales</p><p className="mt-2 text-2xl font-semibold">{salesLabel}</p></Card>
          <Card><p className="text-xs uppercase text-stone-400">Orders</p><p className="mt-2 text-2xl font-semibold">{s.orders}</p></Card>
          <Card><p className="text-xs uppercase text-stone-400">Threads</p><p className="mt-2 text-2xl font-semibold">{s.threads}</p></Card>
        </div>
        <Card>
          <p className="text-sm text-stone-500">{s.products} products in catalog</p>
          <button type="button" className="mr-2 mt-3 inline-flex rounded-full border px-4 py-2 text-sm" onClick={() => { seedDemo(); location.reload(); }}>Load demo</button>
