...
export default function InboxPage() {
  ...
  return (
    <>
      <PageHeader title="Inbox" desc="One thread per customer." />
      <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search threads" className="mb-3 w-full rounded-full border px-4 py-2 text-sm" />
      ...
      <div className="grid gap-3 lg:grid-cols-[280px_1fr]">
        <Card className="p-0">{visible.map((t) => (
          <button key={t.id} type="button" onClick={() => { setActive(t.id); setThreads(markRead(t.id)); }} className={`block w-full border-b px-4 py-3 text-left ${t.id === active ? "bg-brand-tint" : ""}`}>
            ...
            <>
              <div className="flex items-center justify-between border-b px-4 py-3 text-sm font-medium">
                ...
                <div className="flex gap-2">
                  <button type="button" className="rounded-full border px-3 py-1 text-xs" onClick={() => setThreads(markUnread(thread.id))}>Unread</button>
                  <button type="button" className="rounded-full border px-3 py-1 text-xs" onClick={() => {
                    ...
                    setThreads(replyToThread(thread.id, "agent", "Draft " + order.id + " created."));
                  }}>Draft order</button>
                </div>
              </div>
              <div className="flex-1 space-y-2 overflow-y-auto p-4">{thread.messages.map((m) => (
                <div key={m.id} className={`max-w-[80%] rounded-2xl px-3 py-2 text-sm ${m.from === "customer" ? "bg-stone-100" : m.from === "ai" ? "ml-auto bg-brand-tint" : "ml-auto bg-stone-900 text-white"}`}>{m.text}</div>
              ))}</div>
              <div className="flex gap-2 border-t p-3">
                <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send("agent")} className="flex-1 rounded-full border px-3 py-2 text-sm" />
