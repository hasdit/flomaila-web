export const PRODUCT_CSV_HEADER =
  "sku,title,description,category,price,stock,currency";
...
export function upsertProduct(input: Omit<Product, "id" | "createdAt"> & { id?: string }) {
  ...
  const existing = items.find(
    (p) => p.sku.toLowerCase() === sku.toLowerCase() || (input.id && p.id === input.id)
  );
  const next: Product = {
...
export function parseProductCsv(text: string): { ok: Product[]; errors: string[] } {
  ...
  for (const r of ["sku", "title", "price"]) {
    if (!header.includes(r)) errors.push(`Missing column: ${r}`);
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!sku || !title || Number.isNaN(price)) {
      errors.push(`Row ${i + 1}: sku, title, and numeric price are required`);
      continue;
    }
    ok.push({
      ...
      currency: ((cols[idx("currency")] ?? "USD").trim() || "USD").toUpperCase(),
