...
export type Order = { id: string; threadId?: string; customer: string; channel: string; status: OrderStatus; lines: OrderLine[]; createdAt: string; note?: string };
...
export function draftOrder(input: Omit<Order, "id" | "createdAt" | "status">) {
  ...
  saveOrders([order, ...loadOrders()]); return order;
}
...
export function setNote(id: string, note: string) {
  const next = loadOrders().map((o) => (o.id === id ? { ...o, note } : o));
  saveOrders(next); return next;
}
